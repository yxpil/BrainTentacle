...
DNF = "yxpil/dnf-repo"
DL = "https://github.com/yxpil/BrainTentacle/releases/download/{v}/{f}"
...
CASK_TPL = '''cask "bit" do
  ...
  url "https://github.com/yxpil/BrainTentacle/releases/download/v#{{version}}/BIT_#{{version}}_#{{arch}}.dmg"
  name "BIT"
  desc "Local-first AI agent hub with MCP, tool registry and skills"
  homepage "https://github.com/yxpil/BrainTentacle"
...
SCOOP_TPL = '''{{
    ...
    "architecture": {{
        "64bit": {{
            "url": "https://github.com/yxpil/BrainTentacle/releases/download/v{version}/BIT_{version}_x64-portable.zip",
            "hash": "{hash}"
        }}
    }},
    "bin": "BIT.exe",
    ...
    "autoupdate": {{
        "architecture": {{
            "64bit": {{
                "url": "https://github.com/yxpil/BrainTentacle/releases/download/v$version/BIT_$version_x64-portable.zip"
            }}
        }}
    }}
}}
...
def main():
    ...
    # 校验 release 存在且非预发布
    rel = gh("GET", f"/repos/yxpil/BrainTentacle/releases/tags/{a.version}", a.token)
