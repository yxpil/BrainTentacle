...
function install() {
  ...
  if (r.status !== 0) {
    console.error("[bit-agent] 下载失败，请到 https://github.com/yxpil/BrainTentacle/releases 手动下载");
...
if (!binary) {
  console.error("[bit-agent] 无法定位 BIT 应用，请到 https://github.com/yxpil/BrainTentacle/releases 手动下载");
  process.exit(1);
}
